...
src_cls_set = unique(b1_meta$x)
shr_mask = b2_meta$cell_label %in% src_cls_set
sum(shr_mask)

n_corr = length(which(atac.obj$predicted.id[shr_mask] == atac.obj$cell_type[shr_mask]))
n_incorr = sum(shr_mask) - n_corr

n_corr / (n_corr + n_incorr)

dir.create('/home/yxh/gitrepo/multi-omics/seurat/outputs/')
write.csv(mcasubset.atac.pred, file='/home/yxh/gitrepo/multi-omics/seurat/outputs/MCAOS.csv', quote=F, row.names=T)
Matrix::writeMM(obj = as(mcasubset.atac2.umap, 'sparseMatrix'), file="/home/yxh/gitrepo/multi-omics/seurat/outputs/mcaos-atac-umap.mtx")